...
- 2026-03-05: `wt cleanup --apply`/`wt remove --force`에서 `git worktree remove --force`가 권한 거부(`Permission denied`, `허가 거부`)로 실패하면 대상 worktree에 사용자 쓰기 권한(`u+w`)을 복구한 뒤 1회 재시도하도록 개선했다. 권한 계열이 아닌 실패는 기존처럼 즉시 에러를 반환한다.
