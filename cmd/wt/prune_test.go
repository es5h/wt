...
`) + "\n"
	...
	if got[0]["action"] != "would-prune" {
		t.Fatalf("action = %#v, want would-prune", got[0]["action"])
	}
	if got[0]["applied"] != false {
		t.Fatalf("applied = %#v, want false", got[0]["applied"])
	}
	if got[0]["removed"] != false {
		t.Fatalf("removed = %#v, want false", got[0]["removed"])
	}
	if got[0]["reason"] != "gitdir file points to non-existent location" {
		t.Fatalf("reason = %#v, want prune reason", got[0]["reason"])
	}
}

func TestPrune_JSONApply(t *testing.T) {
	t.Parallel()
...
`) + "\n"
	...
	root.SetOut(&stdout)
	root.SetErr(&stderr)
	root.SetArgs([]string{"prune", "--apply", "--json"})
	root.SetContext(context.WithValue(context.Background(), depsKey{}, &deps{
		Runner: &fakeRunner{
			t: t,
			calls: []fakeCall{
	...
	if got[0]["action"] != "pruned" {
		t.Fatalf("action = %#v, want pruned", got[0]["action"])
	}
	if got[0]["applied"] != true {
		t.Fatalf("applied = %#v, want true", got[0]["applied"])
	}
	if got[0]["removed"] != true {
